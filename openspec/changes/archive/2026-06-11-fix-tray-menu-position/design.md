...
Windows 시스템 트레이의 숨겨진 아이콘 영역(접힘 메뉴)에 아이콘이 들어있을 때, 아이콘을 우클릭하면 컨텍스트 메뉴가 마우스 포인터 위치가 아닌 엉뚱한 화면 구석에 렌더링되는 문제가 발생하고 있습니다. 

특히 이 현상은 **처음 우클릭 했을 때만 발생하고 두 번째부터는 정상적으로 표시되는 특징**이 있습니다. 이는 WPF의 `ContextMenu`가 생애 처음 열릴 때 내부 UI 요소의 크기가 아직 계산(Measure/Arrange)되지 않아, 화면 경계를 벗어나는 것을 방지하는 기본 위치 보정 로직이 오작동하기 때문입니다. 한 번 렌더링된 후에는 크기가 캐시되어 두 번째부터는 정상적인 위치에 뜨게 됩니다. 숨겨진 팝업 윈도우 내에서의 상대 좌표 계산 문제와 겹쳐 더욱 두드러지게 나타나는 WPF의 전형적인 이슈입니다.
